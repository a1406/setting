...
namespace MINIW
{
	extern void LogSetCurParam(const char* pfilename, const char* pfunction, int line, unsigned int level);
	extern void    LogMessage(const char* format, ...);
};

#define LOG_INFO        MINIW::LogSetCurParam("", "", 0, 2),     MINIW::LogMessage
...
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "Raknet", __VA_ARGS__))
#define LOGE(...) ((void)__android_log_print(ANDROID_LOG_ERROR, "Raknet", __VA_ARGS__))
#endif 
	...
	if (dhf.isACK)
	{
		...
		for (i=0; i<incomingAcks.ranges.Size();i++)
		{
			...
			for (datagramNumber=incomingAcks.ranges[i].minIndex; datagramNumber >= incomingAcks.ranges[i].minIndex && datagramNumber <= incomingAcks.ranges[i].maxIndex; datagramNumber++)
			{
				...
				LOG_INFO("jackdebug: get ack, number = %d", datagramNumber.val);
...
// 				}
	...
	else if (dhf.isNAK)
	{
		...
		for (i=0; i<incomingNAKs.ranges.Size();i++)
		{
			...
			{
				LOG_INFO("jackdebug: get nak, number = %d", messageNumber.val);
	...
	else
	{
		...
		{
			LOG_INFO("jackdebug: recv2 len %d, reliability = %d, sendReceiptSerial = %d, reliableMessageNumber %d, orderingIndex %d splitPacketCount %d, splitPacketId %d, splitPacketIndex %d",
...
// 					printf("waiting on reliableMessageNumber=%i holeCount=%i datagramNumber=%i\n", receivedPacketsBaseIndex.val, holeCount.val, dhf.datagramNumber.val);
				...
				*/
				...
				if(internalPacket->splitPacketCount > maxPacketSplit) //fix out of memory crash while internalPacket->splitPackageCount is too big for some reason
				{
					FreeInternalPacketData(internalPacket, _FILE_AND_LINE_ );
					ReleaseToInternalPacketPool( internalPacket );
...
// 							printf("RESEND reliableMessageNumber %i with datagram %i\n", internalPacket->reliableMessageNumber.val, congestionManager.GetNextDatagramSequenceNumber().val);
						...
						LOG_INFO("jackdebug: resend size %d, sendtime %d, reliableMessageNumber %d, orderingIndex %d splitPacketCount %d, splitPacketId %d, splitPacketIndex %d, retransmissionTime %d, nextActionTime %d",
							BITS_TO_BYTES(internalPacket->dataBitLength + internalPacket->headerLength),
...
// 						printf("SEND reliableMessageNumber %i in datagram %i\n", internalPacket->reliableMessageNumber.val, congestionManager.GetNextDatagramSequenceNumber().val);
		...
		for (unsigned int datagramIndex=0; datagramIndex < packetsToSendThisUpdateDatagramBoundaries.Size(); datagramIndex++)
		{
			...
			while (msgIndex < msgTerm)
			{
				...
				LOG_INFO("jackdebug: send2 size %d, reliability = %d, sendtime %d, sendReceiptSerial %d, reliableMessageNumber %d, orderingIndex %d splitPacketCount %d, splitPacketId %d, splitPacketIndex %d, retransmissionTime %d, nextActionTime %d",
					BITS_TO_BYTES(packetsToSendThisUpdate[msgIndex]->dataBitLength),
					packetsToSendThisUpdate[msgIndex]->reliability,
					packetsToSendThisUpdate[msgIndex]->timesSent,
					packetsToSendThisUpdate[msgIndex]->sendReceiptSerial,
					packetsToSendThisUpdate[msgIndex]->reliableMessageNumber.val,
					packetsToSendThisUpdate[msgIndex]->orderingIndex.val,
					packetsToSendThisUpdate[msgIndex]->splitPacketCount,
					packetsToSendThisUpdate[msgIndex]->splitPacketId,
					packetsToSendThisUpdate[msgIndex]->splitPacketIndex,
					packetsToSendThisUpdate[msgIndex]->retransmissionTime,
					packetsToSendThisUpdate[msgIndex]->nextActionTime);
			...
			LOG_INFO("jackdebug: send3, len = %d", updateBitStream.GetNumberOfBytesUsed());
...
InternalPacket* ReliabilityLayer::CreateInternalPacketFromBitStream( RakNet::BitStream *bitStream, CCTimeType time )
{
	...
	unsigned short s = 0; bitStream->ReadAlignedVar16((char*)&s); internalPacket->dataBitLength=s; // Length of message (2 bytes)
	if ( internalPacket->reliability == RELIABLE ||
		internalPacket->reliability == RELIABLE_SEQUENCED ||
		internalPacket->reliability == RELIABLE_ORDERED
...
// 		bitStream->PrintBits();
		...
		/*if( internalPacket->splitPacketCount <0 || internalPacket->splitPacketCount >= 1000 ){ 
...
// 		bitStream->PrintBits();
	...
	//fix _RakMalloc_Ex out of memory crash while packageSize is too big for some reason
	const int maxPacketSize = 1024 * 1024 * 4;
	const int maxPacketSplit = (maxPacketSize + (MINIMUM_MTU_SIZE - 1)) / MINIMUM_MTU_SIZE;
	const int packageSize = BITS_TO_BYTES( internalPacket->dataBitLength );
	...
	AllocInternalPacketData(internalPacket, packageSize, false, _FILE_AND_LINE_ );
